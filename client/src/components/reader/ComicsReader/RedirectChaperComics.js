...
const RedirectChaperComics = () => {
  ...
  const handleChange = (values) => {
    ...
    navigation(`/reader/detailComic/${encodedIdStory}/${encodedIdselected}`);
  };
  ...
  const handleNextChapter = () => {
    if (currentIndex < totalChapters) {
      ...
      if (nextChapter) {
        navigation(`/reader/detailComic/${encodedIdStory}/${encodednextChapterId}`);
      }
    }
  };
  const handlePreChapter = () => {
    if (currentIndex > 1) {
      ...
      if (preChapterId) {
        ...
        navigation(`/reader/detailComic/${encodedIdStory}/${encodedpreChapter}`);
      }
    }
  };
  return (
...
export default RedirectChaperComics;
